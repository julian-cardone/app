...
starting mode when beginning any unit of work. Following this procedure consistently reduces
context bleed between tasks and ensures agent work is always isolated.
...
1. Check out a branch following the naming convention in
...
entry point ensures a human reviews intent before any file is modified, which is consistent with
the principle that agents do not take irreversible actions without confirmation.
...
operations, including worktree creation, are listed in [Agent Capabilities](../agents/capabilities.md).
Constraints on irreversible actions are defined in [Agent Constraints](../agents/constraints.md).
